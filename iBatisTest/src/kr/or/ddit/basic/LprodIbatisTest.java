...
//		System.out.println("삭제할 Lprod_gu입력: ");
//		String gu = scan.next();
//		
...
//		System.out.println("출력 끝...");
		...
		//2) 2-4-1. select 작업2 
		//select의 결과 가 1개의 레코드일 경우
		System.out.println("select 작업(결과가 1개의 레코드일 경우...)");
		System.out.println("검색할 Lprod_gu입력: ");
		String gu = scan.next();
		
		// 결과가 1개의 레코드일 경우 형식) smc.queryForObject("namespace값.id값", 파라미터클래스);
		// 반환값: - 처리결과가 여러개일 경우 ==> Exception객체 반환
		// 		 - 처리결과가 1개일 경우 ==> 해당객체 반환 (정상처리)
		//		 - 처리결과가 없을 경우 ==> null반환
		
		//object로 반환되므로 형변환 해야함
