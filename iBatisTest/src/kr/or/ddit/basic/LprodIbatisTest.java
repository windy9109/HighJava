...
public class LprodIbatisTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// sqlMapClient객체변수를 이용해서 처리할 쿼리문을 
			// 호출하여 실행한다.
			...
			*/
			...
			//	반환값 ==> VO객체가 저정된 List객체
			List<LprodVO> lprodList = 
					smc.queryForList("lprod.getAllLprod");
			...
			*/
			...
			// 결과가 1개일 경우 형식) 
			//	  smc.queryForObject("namespace값.id값", 파라미터클래스);
			// 	반환값 : - 처리결과가 여러개일 경우 ==> Exception객체 반환
			//		   - 처리결과가 1개일 경우 ==> 해당객체 반환 (정상처리)
			//		   - 처리결과가 없을 경우 ==> null 반환
			LprodVO lvo4 = 
				(LprodVO) smc.queryForObject("lprod.getLprod", gu);
			if(lvo4==null) {
